...
- **まちのうわさ完成**（2026-09-04）— `bin/uwasa.ts`。776週（2004-W52 〜
  2026-W36）。おつげが「その週に何が起きたか」なら、うわさは「ここまでで
  何が積み上がったか」。節は **つもったもの**（累計＝ステータスの差分）と
  **まだ書いていないもの**（ブログのネタ）の2つだけ —— それ以外はおつげが
  既に書いているので歌わない。素材の読み方（`scan` / `fold`）は
  `otsuge.ts` から借りる。城・つよさ・ルーラ・定時便に配線済み。
...
34. **うわさの累計は「拠点にあるもの」の累計であって、起きたことの累計では
    ない。** ぼうけんのしょは 30日で消えたぶんが手元に無いので、写しを取り
    始める前の週は 0 のまま積み上がらない（実測: 保全を始めた週に 262 さつ
    が一気に入った）。記事とコミットは最初から全部あるので素直に読める。

35. **素材の読み方を2つ持つと、同じ週について違う数を言う部屋ができる。**
    うわさはおつげの `scan` / `fold` をそのまま借りる。借りる側ができたので
    `otsuge.ts` は `import.meta.main` で守った（守らないと import した瞬間に
    776週を書き直しにいく）。
...
- **ふくろの人物・概念。** いまのふくろはプロジェクト台帳だけ
  ...
  増やさずどこまでやるかを決めてから。
- **おつげとうわさの配達。** いまは `otsuge/` と `uwasa/` に置くだけ。
  外に出す口は、掟4に並ぶ
...
まとめて流すなら `bin/yorunotobari.ts`。順番に意味がある（ふくろ・
ステータス・おつげ・うわさは拠点に書かれたものを畳むので必ず後ろ。
うわさはおつげの `scan` / `fold` を借りるので、おつげの次）。
...
bin/tsuyosa.ts --plain うわさ        # 累計が積み上がって見えるか
...
bin/uwasa.ts                # まちのうわさ。週ごとの累計とネタ（同上）
