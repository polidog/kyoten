...
function main(): number {
  ...
  //
  // **ここだけは「全機械の素材が揃った日」の門を建てない。** 日記や
  // よその日記と違って、この部屋が書くのは「その日の記録」ではなく
  // 「きょうの話題のうち、どれが近いか」。素材（`ニュース/`）はどの機械でも
  // 同じで、拠点の側（`事典/` `週報/`）は背景にしか使わない。しかも
  // 埋め戻さない部屋なので、門を建てると**相手の機械が1日遅れているあいだ
  // ずっと黙る**ことになる。守れるものが小さく、失うものが大きい。
