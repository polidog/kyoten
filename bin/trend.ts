 ...
 * trend — プロフィールの推移（週ごと）
 *
 * 週報が「その週に何が起きたか」を書くのに対して、推移は
 * **ここまでで何が積み上がったか**を書く。プロフィールは「いまの自分」を
 * 1枚だけ持っていて、先週の自分は残らない。週ごとに置いておけば、
 * あとから「2026年9月の自分」を読める。だから置き場もプロフィールの下。
 *
 * 節は2つだけにしてある。
 *
 *     つもったもの        その週の終わりまでの累計（プロフィールの差分）
 *     まだ書いていないもの  polidog.jp に名前が出ていないもの（ブログのネタ）
 *
 * 「何に時間を使い、何が止まったか」は週報がすでに書いている。同じことを
 * 別の部屋でもう一度言うと、どちらも読まれなくなる。
 *
 * 「まだ書いていないもの」は記録ではなく**提案**なので、本当はアイボの
 * 仕事。アイボの部屋ができたらそちらへ移す（それまではここに置いておく）。
 *
 * 素材の読み方は週報から借りる（`scan` / `fold`）。同じ週について違う数を
 * 言う部屋を作らないため。走らせる順番は … → profile → weekly → trend。
 *
 * 原則:
 *   - 決定論的: 走らせた日で結果が変わらない。
 *   - 冪等: 内容が変わらなければファイルに触れない。
 *   - その週の目でだけ書く: 累計もネタも、その週の終わりまでしか知らない
 *     （落とし穴22）。未来を知らない推移は二度と変わらない。
 *
 * 使い方:
 *     trend.ts                # 全部
 *     trend.ts --dry-run
...
import { Day, Week, fold, scan } from "./weekly.ts";

const ROOM = join(KYOTEN, "プロフィール", "推移");

/** 累計コミットのふしめ。跨いだ週にだけ書く。 */
const MILESTONES = [10, 50, 100, 500, 1000, 5000] as const;

/**
 * ネタとして名前を照らす下限の長さ。
 *
 * 落とし穴18 と同じ話で、短い名前はただの単語になる。`polidog/web` の `web` を
 ...
 *
 * 投稿のうち `source: polidog.jp` のものだけ見る。SNS を混ぜると
...
function whenWritten(names: ReadonlySet<string>): Map<string, string> {
  ...
  if (!existsSync(posts) || !names.size) return first;
...
function repoName(project: string): string {
  if (!project.includes("/")) return ""; // 落とし穴18: `Work` や `_home` はただの単語
...
function render(week: Week, t: Total, fresh: number, neta: readonly Neta[]): string {
  ...
  const body: string[] = [`# ${week.key} の推移`, `${week.start} 〜 ${week.end}`];

  const tsumotta: string[] = [];
  if (t.sessions) tsumotta.push(`- 会話 ${grew(t.sessions, week.sessions, "本")}`);
...
function main(): number {
  ...
  for (const week of weeks) {
    ...
    for (const [project, count] of week.projects) {
      ...
      const repo = repoName(project);
      if (!repo) continue;
      // その週の終わりまでに書かれていれば、ネタではない（落とし穴22: 未来を見ない）
  ...
  if (args.flags.quiet) {
    console.log(
      `trend: ${total}週 (new ${stats.new} / upd ${stats.updated} / same ${stats.same})` +
        ` ネタ ${n(netaTotal)}`,
    );
  } else {
    if (args.flags["dry-run"]) console.log("（書かずに確認）");
    console.log(`  推移       : ${n(total)} 週`);
