...
上の8本が拠点に書き、`search.ts` が索引を作り、下の3本は**読むだけ**。
書き手を1つに保つため（原則5）、読む道具は拠点に一切触れない。

素材は下から上へ流れる。1階（会話・自分・投稿・作業）は外から集め、
**2階（事典・プロフィール・週報・推移）は拠点の中しか見ない** —— jsonl も
git も直接は読まず、他の部屋が書いたものを畳み直す。だから走らせる順番は
sessions → me → posts → work → entities → profile → weekly → trend で
固定になる。
...
- 刻む対象は `会話/` `自分/` `投稿/` `作業/` `事典/` `プロフィール/` `週報/`、それと読み専用の水源
...
1. **`アイボ/`（AI の行動記録）** — いまの拠点は8部屋あって全部 polidog の
   ことしか書いていない。相棒は隣の眼なので、自分が何をしたかを持っていないと
   ただの鏡になる。素材は `会話/` にもう入っている（`replies` と `models`、
   見出しは `## HH:MM:SS assistant（model）`）ので、`自分/` の鏡像を1本書けば取れる
