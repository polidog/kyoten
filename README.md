...
| `shiro.ts` | 拠点をブラウザで歩く（読むだけ） |
| `tsuyosa.ts` | 拠点を端末で歩く（読むだけ） |
...
Obsidian で拠点を開くと、見えるのは日付順に並んだテキストになる。23年ぶんの
数はステータス画面に、とくぎ71枚は★の一覧に、おつげ776週は年ごとの帯にした
ほうが速く見つかる。城はその見せ方だけを持っていて、拠点には**一切書かない**。

- `node:http` だけ。npm も build も無い
- **127.0.0.1 に固定**。`--host` は用意していない —— 拠点には取引先の実名も
  Slack のスレッド全文も入っている。外向きに開ける口を最初から作らない
- 画面は `bin/shiro.html` 1枚（CSS も JS も中にある）。リクエストのたびに
  読み直すので、立ち上げたまま直してリロードすれば効く
- 部屋は つよさ / とくぎ / ねんぴょう / おつげ / ふくろ / ルーラ。ルーラは
  打つそばから引いて、結果から原文まで開ける
- `↑↓` でコマンドを選び、`/` でルーラへ飛ぶ

素材はぜんぶ frontmatter と表から読む（`bin/yomi.ts`）。数を出すところだけ
パースして、本文は**見出しで切ったまま**渡す。箇条書きの書式に賭けて壊すより、
原文をそのまま見せるほうが掟3に合う。

### 拠点の中身は必ずエスケープして描く

会話原文には HTML も `<script>` も普通に入っている（人のコードの話をして
いるのだから当たり前）。描くのは見出し・箇条書き・表・コードフェンスだけに
絞って、中身は全部エスケープを通す。検索語を光らせるところも、印を入れてから
エスケープすると印まで消えるので、一致箇所で切って組み立てる。

原文を開く口（`/api/raw`）は拠点の下に正規化して収める。`..` で外に出ようと
したものは読まない。ローカル専用とはいえ、住所を組み立てるのはブラウザ側なので。
