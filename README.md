...
`bin/dougu.ts`、引数まわりは `bin/cli.ts`、拠点を読む層は `bin/yomi.ts`。
...
| `uwasa.ts` | おつげと同じ素材 → 週ごとの累計とブログのネタ |
| `ruula.ts` | 上を全部と reading-notes を全文検索 |
| `yorunotobari.ts` | 順に流して拠点をきょうかいする定時便 |
| `shiro.ts` | 拠点のまとめをブラウザに出す（読むだけ） |
| `tsuyosa.ts` | 拠点を端末で歩く。潜って読むのはこちら（読むだけ） |
| `suzu.ts` | すずのおと。hooks から鳴って、拠点を思い出させる |

上の7本が拠点に書き、`ruula.ts` が索引を作り、下の2本は**読むだけ**。
書き手を1つに保つため（掟5）、見る道具は拠点に一切触れない。
...
おつげが「その週に何が起きたか」なら、うわさは**ここまでで何が積み上がったか**。

ステータスは「いまの自分」を1枚しか持っていない。先週の自分は上書きされて
...
設計図の吟遊詩人は「何に時間を使い、何が増え、何が止まったか」を歌うことに
なっているが、**それはおつげがすでに全部書いている**。同じことを別の部屋で
もう一度言うと、どちらも読まれなくなる。

だから残したのは、おつげに無い2つだけ。**つもったもの**（ステータスの差分）と、
**まだ書いていないもの**（ブログのネタ）。

### 素材の読み方はおつげから借りる

`scan()`（日ごとに拾う）と `fold()`（週に畳む）は `otsuge.ts` から export して
共有している。読み方が2つに割れると、**同じ週について違う数を言う部屋**が
できる。`otsuge.ts` は `import.meta.main` で守ってあるので、import しても
走らない。

### 累計は「拠点にあるもの」の累計

**実際に起きたことの累計ではない。** ぼうけんのしょは 30日で消えたぶんが
手元に無いので、写しを取り始める前の週は 0 のまま積み上がらない。上の例で
`ぼうけんのしょ 335さつ（+262）` と1週で 262 も増えているのはそのため
...
照らすのは `<user>/<repo>` の `<repo>` だけで、4文字未満は落とす（罠18 と
...
おつげと同じ（罠22）。累計もネタも、その週の終わりまでしか知らない。
「まだ書いていない」かどうかも、その週の終わりまでに出た記事だけで決める。
未来を知らないうわさは二度と変わらないので、776週ぶんが完全に冪等になる。

```bash
bin/uwasa.ts              # 全部
bin/uwasa.ts --dry-run
bin/uwasa.ts --quiet
```
...
```bash
bin/shiro.ts                # 立ち上げてブラウザを開く
bin/shiro.ts --port 9999
bin/shiro.ts --no-open
```

Obsidian で拠点を開くと、見えるのは日付順に並んだテキストになる。城は
23年ぶんを**1枚のまとめ**にして返す。上から下へ読むもので、一覧も検索も
持たない —— 776週のおつげや71枚のとくぎを潜って読むのは端末の仕事。

出るのは、ステータス（つよさ・とくぎ・おつげの数）、**今週のおつげ**、
とくぎの上から12枚、いま手が動いているものと長くいっしょにいるもの、
23年のあゆみ、最近いた場所の8つ。おつげだけは節を原文のまま出す ——
向こうから来た問いは要約しない。

- 口は `/api/summary` の1本だけ。画面は `bin/shiro.html` 1枚（CSS も JS も
  中にある）。リクエストのたびに読み直すので、立ち上げたまま直せる
- **127.0.0.1 に固定**。`--host` は用意していない —— 拠点には取引先の実名も
  Slack のスレッド全文も入っている。外向きに開ける口を最初から作らない
- 拠点には**一切書かない**。素材は `bin/yomi.ts` から取る

見た目は[設計図](https://claude.ai/code/artifact/08ea4f4f-4ebf-4764-a404-69a1fb8ed669)に
合わせてある。明るい地に白いカード、2px の線と `0 2px 0` の段差、見出しは
DotGothic16。ドラクエの黒いウィンドウは**全面に広げず、要所のアクセント**に
だけ使う（ステータス画面のところ）。CSP は外を向くのを字の形だけに絞って
あり、拠点の中身が外へ出る口はない。
...
```bash
bin/tsuyosa.ts                    # コマンドウィンドウを開く
bin/tsuyosa.ts とくぎ php          # その部屋・その行き先から始める
bin/tsuyosa.ts --plain おつげ      # 対話せず1回だけ描く（パイプ用）
bin/tsuyosa.ts --plain ルーラ 冪等
```

ドラクエのコマンドウィンドウ。`↑↓` でえらんで、`→` で入って、`←` で戻る。
城と同じものを見るが、ブラウザを開かずに済む。中身は `bin/yomi.ts` を共有
していて、こちらも拠点には書かない。

端末の幅は自前で数える。`.length` は UTF-16 の単位なので、日本語を並べると
枠が崩れる（罠24 と同じ話）。East Asian Width の表を丸ごと持つと重いので、
...
2. **描くのを最後の1回にまとめると、キーの処理が古い画面を見る。** 塊で
   届いた `Enter` が、前の画面のカーソル行を開いた（ルーラの結果を開いた
   つもりで、ステータス画面の1行目にあった `typescript` が開いた）。
...
2階まで建てても、こちらから見に行かないと何も返ってこない。それは掟4で
...
| いつ | 何をするか |
|---|---|
| `SessionStart` | 拠点の大きさ・いま居る場所のふくろ・いちばん新しいおつげ・ルーラの呼び方を配る |
| `UserPromptSubmit` | 過去を指す言い回しを見つけたらルーラを促す |
| `SessionEnd` | 終わったその場でぼうけんのしょに写す（夜まで待たない） |

```bash
echo '{"hook_event_name":"SessionStart","cwd":"/home/polidog/ghq/github.com/polidog/kyoten"}' | bin/suzu.ts
...
```json
{
  "hooks": {
    "SessionStart": [
      { "matcher": "*",
        "hooks": [{ "type": "command", "command": "\"$HOME/ghq/github.com/polidog/kyoten/bin/suzu.ts\"", "timeout": 5 }] }
    ],
    "UserPromptSubmit": [
      { "hooks": [{ "type": "command", "command": "\"$HOME/ghq/github.com/polidog/kyoten/bin/suzu.ts\"", "timeout": 5 }] }
    ],
    "SessionEnd": [
      { "hooks": [{ "type": "command", "command": "\"$HOME/ghq/github.com/polidog/kyoten/bin/suzu.ts\"", "timeout": 30 }] }
    ]
  }
}
```

外すときはこの3つを消すだけ。すずは拠点に書かないので、消しても何も残らない
（SessionEnd の写しだけは utsushi が書くが、それは夜の便が書くものと同じ）。

### 鳴らないほうがましなので、何があっても 0 で終わる

すずが鳴らないのは構わない。すずのせいでセッションが止まるのがいちばん困る。
...
すずは**全プロジェクトのセッション**で鳴る。仕事のリポジトリでも鳴る。
だから配るのは、おつげの見出しと数、ふくろの台帳1行、ルーラの呼び方まで。
**会話の原文は渡さない**（そこに取引先の実名もメールアドレスも入っている）。
原文が要るときは、賢者がルーラを叩いて自分で取りに行く。

### 過去参照は実測 1.5% でしか鳴らない

`UserPromptSubmit` の語彙は、ことのは 609発言に当てて決めた。
...
`あの話` `例の` `以前` `前回` `このまえ` `昔` はどれも 0 件だった。7日ぶんの
ことのはしか無いので「使わない語」と決めつけず、precision の高いものは残して
ある。**入れなかったのは `前に＋動詞`** —— 「この行の前に書いて」のような
指示文と見分けがつかず、鳴りすぎるとすぐ読み飛ばされるようになる。

鳴る回数が少ないのは負けではない。毎プロンプト鳴るすずは、鳴っていないのと
同じになる。

### いま居る場所は cwd から引く

`SessionStart` の payload には `cwd` が入っている。`slugFromCwd()` で
`<user>/<repo>` にしてふくろを引くと、「このリポジトリについて拠点は何を
憶えているか」が最初の1行で返る。プロンプトの中の固有名詞を探すより、
これは確実で、しかも1セッションに1回しか走らない。

ふくろの名前で照らし合わせるのは `/` を含むものだけにしてある（罠18 ——
...
19. **「何があっても 0 で終わる」は `main` の中だけでは守れない**
    ...
    ```bash
    echo '{"hook_event_name":"UserPromptSubmit","prompt":"あの話だっけ"}' | bin/suzu.ts
    echo "終了コード $?"   # 0 でなければ登録しない
    ```

20. **`SessionEnd` の写しは utsushi に 1本だけ渡す口が要る**
    `utsushi.ts --file <jsonl>` を足した。名前の衝突（罠1）はこの口では
